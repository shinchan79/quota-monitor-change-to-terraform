...
resource "aws_lambda_function" "lambda" {
  ...
  # Source code logic:
  # 1. Nếu create_s3 = true: dùng filename từ source path của s3_objects
  # 2. Nếu create_s3 = false: 
  #    - Nếu có filename thì dùng filename (local file)
  #    - Nếu không có filename thì dùng s3_bucket và s3_key (existing S3)
